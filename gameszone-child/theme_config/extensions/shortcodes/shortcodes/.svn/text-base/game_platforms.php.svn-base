...
{
    extract(shortcode_atts(array('cat' => '','title' => '','img' => '','desc' => '',
    		'link' => '','email' => '','caption' => '','button'=>'', 'bootstrap' => ''), $atts));
    ...
    if(!empty($categories))
    {
		...
		$tmp_title = !empty($link) ? '<a href="'.$link.'">'.$title.'</a>':$title;
        $link = !empty($link)? $link : "?s=~&amp;tax_games='.$id_term.$platforms_links.'";
        $caption = !empty($caption)? $caption : "View Games";
       	$bootstrap = !empty($bootstrap) ? $bootstrap : "col-sm-4";
        $out .= '
                <div class="'.$bootstrap.'">
	                   ...
	                   <div class="textbox textbox-style2">
        					<div class="textbox-container">';
	                                ...
	                                $out .='<div class="textbox-aside">
	                                        <h3 class="textbox-title">'.$tmp_title.'</h3>
	                                        <div class="textbox-text">
	                                                <p>'.$desc.'</p>
	                                        </div>';
	                                		 if(empty($button)):
	                                         if(empty($email) ):
	                                    $out .=    '<div class="textbox-foot clearfix">
	                            
	                                                <a class="btn btn-default btn-xs" href="'.$link.'">'.__($caption,'tfuse').'</a>
	                                        </div>';
	                                    	else:
	                                         $out .=    '
