...
jQuery(document).ready(function(){
    ...
    $('#add_new_sidebar_name').on('keyup',function(event){
        if(event.keyCode == 13){
            $(this).next('input[type="button"]').click();
        }
    });
    
    $('#add_new_sidebar_submit').on('click',function(){
        $.post(ajaxurl,{
    ...
    $('.sidebars_placeholder_container').on('click',function(){
    ...
    $('.sidebars_placeholder_box li .delete').on('click',function(){
    ...
    if(tf_script && tf_script.current_page && tf_script.current_page=='widgets') {
        ...
        $('#tfuse_fields .sidebar-name').on('click',function(){
        ...
        $('.sidebars_placeholder_box').on('sortstart',function(){
            if($(this).is(':visible'))
                sdb_changes_on();
        });
